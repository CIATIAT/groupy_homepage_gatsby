  ...
  <StaticQuery
    query={graphql`
      query PrivacyPolicyQuery {
        contentfulDocument(text: { childMarkdownRemark: { id: { eq: "c20968b8-d7e8-5812-bf57-74dddf443e9d" } } }) {
          text {
            childMarkdownRemark {
              rawMarkdownBody
            }
    ...
    render={(data) => (
      <Section.Container Background={Background}>
        <Box height={80}></Box>
        <Section.Header name={"プライバシーポリシー"} />
        <Flex justifyContent="center" alignItems="center" flexWrap="wrap">
          <Box width={[1, 1, 3 / 5]} px={[1, 2, 4]}>
            <Fade direction="down" triggerOnce>
              <ReactMarkdown
                children={data.contentfulDocument.text.childMarkdownRemark.rawMarkdownBody}
